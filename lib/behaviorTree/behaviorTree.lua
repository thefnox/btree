...
function BehaviorTree.new(definition: NodeDefinition, blackboard: Blackboard, debug: boolean?): Tree
    ...
    local tree: Tree = {
        ...
        -- Interrupts any running tasks and resets the tree to its initial state.
        stop = function(_self: Tree)
            stopNode(rootIndex, nodes, childrenData, blackboard)
            resetNode(rootIndex, nodes, childrenData)
            -- Clear per-tick tracking so the next update starts with a clean slate and
            -- does not fire spurious onLeave callbacks for tasks that were active before stop().
            for i = 1, #nodes do
                local n = nodes[i]
                if n.definition._type == "task" then
                    n.activeLastTick = false
                    n.activeThisTick = false
                end
            end
            local ctx = treeRegistry[blackboard]
